...
/**
 * This is copy/paste from apache commons net original class (FTPHTTPClient) since it does not supports Socket timeouts.
 * See https://issues.apache.org/jira/browse/NET-538
 * 
 * Please remove it if this will be fixed
 * 
 * Experimental attempt at FTP client that tunnels over an HTTP proxy connection.
 * 
 * @since 2.2
 */
public class LocalFTPHTTPClient extends FTPClient {
  ...
  public LocalFTPHTTPClient( String proxyHost, int proxyPort, String proxyUser, String proxyPass ) {
  ...
  public LocalFTPHTTPClient( String proxyHost, int proxyPort ) {
  ...
  private void tunnelHandshake( String host, int port, InputStream input, OutputStream output ) throws IOException,
    ...
    output.write( CRLF );
    output.write( "Connection: Keep-Alive".getBytes( "UTF-8" ) );
    output.write( CRLF );

    List<String> response = new ArrayList<String>();
    BufferedReader reader = new BufferedReader( new InputStreamReader( input, getCharsetName() ) ); // Java 1.6 can use
                                                                                                    // getCharset()
